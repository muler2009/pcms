  ...
  return (
    <OuterContainer className="flex font-Poppins bg-[#ffffff]">
      <InnerContainer
        ...
        } border-[#ddd] flex flex-col shadow-md`}
      >
          ...
          <div className="flex justify-center items-center">
            ...
            <div
              className={`flex flex-col ${
                !controller && "opacity-0 translate-x-28 overflow-hidden"
              }`}
            >
          ...
          {account?.map((account, index) => {
            return (
              <div className="relative px-3" key={index}>
                <Link
                  className={`flex items-center justify-start space-x-2 py-3 px-5`}
                  to={account.path}
                >
                  <div size={20}>{account.icon}</div>
                  <div
                    className={`duration-500 font-Poppins text-sm ${
                      !controller && "opacity-0 translate-x-28 overflow-hidden"
                    }`}
                  >
                    {account.label}
