...
const DatePickComponent = ({ className, selected, onChange }) => {
  ...
  return (
    <div className="relative flex items-start justify-start space-x-7">
      <label className="whitespace-nowrap">Birth Date</label>
      <div className="input-sm rounded-none">
        <Sl.SlCalender className="inline-flex mr-2" />
        <DatePicker
          className=" text-black font-Poppins focus:outline-none placeholder-transparent border-gray-600 border-gray-30 text-sm "
          renderCustomHeader={({
            date,
            changeYear,
            changeMonth,
            ...
            <div className="flex justify-center items-center space-x-2 py-3 bg-transparent pr-3">
              ...
              <select
                ...
                className="px-1 active:border-none"
              >
                {years.map((option) => (
                  <option
                    key={option}
